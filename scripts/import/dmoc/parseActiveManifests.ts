...
export function parseActiveManifests(filePath: string): ParseResult {
  ...
  try {
    ...
    for (let i = headerIndex + 1; i < lines.length; i++) {
      ...
      // Check if this is a new data row (starts with tab + numeric ID)
      const trimmed = line.trim();
      if (trimmed && /^\d+/.test(trimmed)) {
        // Process previous row if exists
        if (currentRow.length > 0) {
          try {
            const manifest = parseManifestRow(headers, currentRow, currentNarrative);
            if (manifest) {
              manifests.push(manifest);
            }
          } catch (error) {
            errors.push(`Row ${manifests.length + 1}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }
        }
  ...
  return {
    ...
    errors,
    summary: {
      totalRows: manifests.length + errors.length,
 ...
 */
function parseManifestRow(headers: string[], row: string[], narrative: string[]): ParsedManifest | null {
...
if (require.main === module) {
  const filePath = process.argv[2] || join(process.cwd(), 'data', 'active_manifests.txt');
